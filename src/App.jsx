...
const COLLABORATIVE_CALCULATORS = {
  'ASDAS_CRP': {
    ...
    patientDesc: 'Pendiente de completar por reumatólogo/a o enfermera en consulta',
...
const ASDASCalculator = ({ onResult, isDoctor = true }) => {
...
const DAPSACalculator = ({ onResult, isDoctor = true }) => {
  const [components, setComponents] = useState({
...
const DAS28Calculator = ({ onResult, isDoctor = true }) => {
  const [variant, setVariant] = useState('CRP');
  const [components, setComponents] = useState({
...
const SLEDAICalculator = ({ onResult, isDoctor = true }) => {
  const [components, setComponents] = useState({
...
const SSDAICalculator = ({ onResult, isDoctor = true }) => {
  const [components, setComponents] = useState({
...
const SCORE2Calculator = ({ onResult, isDoctor = true }) => {
  const [components, setComponents] = useState({
    age: 55,
    sex: 'male',
    smoking: 'no',
...
const SCORE2OPCalculator = ({ onResult, isDoctor = true }) => {
  const [components, setComponents] = useState({
    age: 75,
    sex: 'male',
    smoking: 'no',
...
const DoctorDashboard = ({ user, onLogout }) => {
  ...
  const renderCalculator = () => (
    <div className="calculator-view">
      <button className="btn-back-calc" onClick={() => { setView('patient'); setSelectedCalc(null); setResult(null); setSaved(false); }}>
        ← Volver al paciente
      </button>
      ...
      {!selectedCalc ? (
        <>
          ...
          <div className="calc-grid">
            <button className="calc-card" onClick={() => { setSelectedCalc('BASDAI'); setResult(null); }}>
              <span className="calc-icon">📊</span>
              <span className="calc-name">BASDAI</span>
              <span className="calc-desc">Actividad espondilitis</span>
            </button>
            <button className="calc-card" onClick={() => { setSelectedCalc('ASDAS'); setResult(null); }}>
              <span className="calc-icon">📈</span>
              <span className="calc-name">ASDAS</span>
        ...
        <>
          {selectedCalc === 'BASDAI' && <BASDAICalculator onResult={handleResult} />}
          {selectedCalc === 'ASDAS' && <ASDASCalculator onResult={handleResult} isDoctor={true} />}
          {selectedCalc === 'DAPSA' && <DAPSACalculator onResult={handleResult} isDoctor={true} />}
          {selectedCalc === 'DAS28' && <DAS28Calculator onResult={handleResult} isDoctor={true} />}
          {selectedCalc === 'SLEDAI' && <SLEDAICalculator onResult={handleResult} isDoctor={true} />}
          {selectedCalc === 'LupusPRO' && <LupusPROCalculator onResult={handleResult} />}
          {selectedCalc === 'FACIT' && <FACITCalculator onResult={handleResult} />}
          {selectedCalc === 'SF36' && <SF36Calculator onResult={handleResult} />}
          {selectedCalc === 'BASFI' && <BASFICalculator onResult={handleResult} />}
          ...
          {selectedCalc === 'PSAQoL' && <PSAQoLCalculator onResult={handleResult} />}
          {selectedCalc === 'ESSPRI' && <ESSPRICalculator onResult={handleResult} />}
          {selectedCalc === 'SSDAI' && <SSDAICalculator onResult={handleResult} isDoctor={true} />}
          {selectedCalc === 'SCORE2' && <SCORE2Calculator onResult={handleResult} isDoctor={true} />}
          {selectedCalc === 'SCORE2-OP' && <SCORE2OPCalculator onResult={handleResult} isDoctor={true} />}
