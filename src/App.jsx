...
const DoctorDashboard = ({ user, onLogout }) => {
  ...
  const handleSaveScore = async () => {
    ...
    // Si estamos completando una pendiente
    if (selectedPending) {
      await Storage.completePendingCalculation(selectedPending.id, user.id);
    }
            ...
            {pendingCalcs.map((pending) => (
              <button
                ...
                onClick={() => {
                  ...
                  setSelectedCalc(pending.instrument.replace('_CRP', '').replace('_ESR', ''));
                  setView('calculator');
                }}
                style={{
                  padding: '0.5rem 1rem',
